// ═══════════════════════════════════════════════════════════
//  routes/tesourariaRoutes.js — Gestão Financeira da Igreja
//  CRUD de transações (entradas e saídas) + relatórios
// ═══════════════════════════════════════════════════════════
...
// ── GET /api/tesouraria — Lista todas as transações (com filtros opcionais) ──
...
// ── POST /api/tesouraria — Registra nova transação ──
router.post('/', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    if (!categoria || !descricao || !valor || !data_transacao) {
      return res.status(400).json({ erro: 'Campos obrigatórios: tipo, categoria, descricao, valor, data_transacao' });
    }
...
// ── PUT /api/tesouraria/:id — Atualiza transação ──
router.put('/:id', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    await pool.query(
      'INSERT INTO auditoria (membro_id, acao, detalhes) VALUES ($1, $2, $3)',
      [req.user.id, 'TESOURARIA_UPDATE', `Atualizou transação ${id}`]
    );
...
// ── DELETE /api/tesouraria/:id — Remove transação ──
router.delete('/:id', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    await pool.query(
      'INSERT INTO auditoria (membro_id, acao, detalhes) VALUES ($1, $2, $3)',
      [req.user.id, 'TESOURARIA_DELETE', `Removeu transação ${id}`]
    );

    res.json({ sucesso: true, mensagem: 'Transação removida' });
  } catch (err) { next(err); }
});

// ── GET /api/tesouraria/resumo — Resumo financeiro (dashboard tesouraria) ──
router.get('/resumo', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    let recentParams = [...params];
    const { rows: recentes } = await pool.query(`
      SELECT * FROM transacoes_financeiras ${recentWhere}
